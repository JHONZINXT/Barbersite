...
const icons: Record<string, JSX.Element> = {
  scissors: (
    <svg width="26" height="26" viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth={2.4}>
          ...
          <div className="flex flex-col gap-4">
            {siteConfig.services.map((s, i) => (
              <motion.div
                ...
                className={`flex items-stretch rounded-2xl overflow-hidden bg-card border ${
                  "featured" in s && s.featured ? "border-amber/60" : "border-amber/15"
                }`}
                style={{ minHeight: 112 }}
              >
                <SlotImage
                  src={s.image}
                  alt={s.name}
                  placeholderLabel={`Foto: ${s.name}\n(ação, quadrada 1024x1024)`}
                  className="w-[104px] shrink-0 rounded-r-[999px]"
                />
                <div className="flex-1 min-w-0 flex flex-col justify-center py-3 px-5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-display uppercase text-lg md:text-xl tracking-wide">{s.name}</span>
                    {"featured" in s && s.featured && (
                      <span className="font-mono text-[9px] uppercase tracking-wide text-ink bg-amber px-2 py-0.5 rounded-full">
                        Mais pedido
                      </span>
                    )}
                  </div>
                  <p className="text-xs opacity-60 leading-relaxed mt-1 max-w-md">{s.desc}</p>
                  <div className="font-display text-lg text-amber mt-1.5">{s.price}</div>
                </div>
              </motion.div>
            ))}
          </div>
